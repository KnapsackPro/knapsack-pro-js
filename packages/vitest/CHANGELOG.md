...
- Include each file's `collectDuration` (import + transform + suite-collection) and `setupDuration` (setup-file import) in the recorded time reported to Knapsack Pro, in addition to test/hook run time. Previously only `testCase.diagnostic().duration` was reported, which excludes import/transform cost that can dwarf the actual test run time.
